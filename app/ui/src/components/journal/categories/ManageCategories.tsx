        ...
        <Paper sx={(theme) => ({ borderRadius: theme.spacing(1) })}>
          ...
          {Object.values(getCategoriesQuery.data).length === 0 ? (
            <Typography align="center" variant="body2" px={2} py={3}>
              No categories.{' '}
              <MuiLink sx={{ pointer: 'cursor' }} onClick={() => setShowCreateCategoryModal(true)}>
                Create one
              </MuiLink>
            </Typography>
          ) : (
                  ...
                  <ListItem
                    ...
                    secondaryAction={
                      <Stack direction="row" gap={1}>
                        <Button color="primary" onClick={() => handleSelectCategoryForEdit(category)}>
                          Edit
                        </Button>
                        <Button color="error" onClick={() => handleDeleteCategory(category)}>
                          Delete
                        </Button>
                      </Stack>
                    }>
