        ...
        <Box>
          {activeSlot ? (
            <>
              ...
              <Box sx={{ px: 2, py: 1, pt: 2 }}>{journalFilterSlots[activeSlot].component}</Box>
            </>
          ) : (
            <>
              <TextField size="small" placeholder="Filter by..." disabled autoFocus sx={{ m: 1, mt: 0 }} />
              {Object.entries(journalFilterSlots).map(([key, properties]) => {
                return (
                  <MenuItem key={key} onClick={() => handleClickSlotButton(key as SearchFacetKey)} dense>
                    <ListItemIcon sx={(theme) => ({ color: theme.palette.text.secondary })}>
                      {properties.icon}
                    </ListItemIcon>
